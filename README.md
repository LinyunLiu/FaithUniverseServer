...
mysql> CREATE USER 'faithuniverse'@'localhost' IDENTIFIED BY 'NEW PASSWORD';
mysql> GRANT ALL PRIVILEGES ON *.* TO 'faithuniverse'@'localhost';
mysql> FLUSH PRIVILEGES;
mysql> exit
sudo service mysql status
systemctl status mysql

# Import Bible to DataBase
mv kjv.csv /var/lib/mysql-files/
# create.sql 
USE Bible;
LOAD DATA INFILE '/var/lib/mysql-files/kjv.csv'
INTO TABLE Kjv
FIELDS TERMINATED BY ','
ENCLOSED BY '"'
LINES TERMINATED BY '\n'
IGNORE 1 LINES
(Id, BookName, BookNumber, Chapter, Verse, Text);
# In the MySQL console
source import.sql

# Add Migration, This will generate the migration files under the Migrations folder in your project.
# Make sure to update connectionStrings in appsettings.json
...
sudo nano faithuniverse.service

# Add the following code
[Unit]
Description=ASP.NET Bible Study Website Server
After=network.target

[Service]
WorkingDirectory=/root/Bible/Bible
ExecStart=/usr/bin/dotnet run --urls "http://xxx.xxx.xxx.xxx:3000"
Restart=always
RestartSec=10
SyslogIdentifier=faithuniverse
User=root
...
sudo systemctl start faithuniverse      # Start the application
sudo systemctl restart faithuniverse    # Restart the application
sudo systemctl stop faithuniverse       # Stop the application
sudo systemctl enable faithuniverse     # Enable it to start on boot
sudo systemctl disable faithuniverse    # Disable the service from starting on boot

sudo systemctl status faithuniverse
```
...
zip -r Bible.zip Bible -x '/folder/path/to/ignore/*'
